...
const RadarMap = ({ flights }) => {
    // Center map approximately around US East Coast as fallback
    const defaultCenter = [40.6413, -73.7781];
    ...
    return (
        <div className="relative w-full h-full bg-slate-950 overflow-hidden">
            <MapContainer
                key={hasLocation ? "user-loc" : "default-loc"} // Force re-render when location is found
                center={mapCenter}
                zoom={hasLocation ? 9 : 7}
                    ...
                    >
                        <Popup className="bg-slate-800 text-slate-100 rounded border border-slate-700 shadow-xl">
                            ...
                            <div className="text-sm">
                                <div><span className="text-slate-400">Alt:</span> {Math.round(flight.altitude)} ft</div>
                                <div><span className="text-slate-400">Speed:</span> {Math.round(flight.speed)} kt</div>
                                <div><span className="text-slate-400">Hdg:</span> {Math.round(flight.heading)}°</div>
                            </div>
                        </Popup>
                    </Marker>
                ))}
